...
-- Class resources only available in wrath
if (IsWrath) then
	ClassPower.Runes = setmetatable({
		...
		UpdateRuneType = function(rune, runeID, alt)
			rune.runeType = GetRuneType(runeID) or alt
			return rune
		end,
		ColorRune = function(self, rune, runeType)
		...
		UpdatePower = function(self, event, unit, ...)
			...
			if (event == "RUNE_TYPE_UPDATE") then
				...
				local runeID, alt = ...
				if (runeID) then
					rune = element.UpdateRuneType(element[element.runeOrder[runeID]], runeID, alt)
					if (rune) then
						local color, r, g, b = element.ColorRune(self, rune, rune.runeType)
			...
			for index, runeID in ipairs(element.runeOrder) do
			...
			return numReady, #element.runeOrder, element.powerType
		end,
		UpdateColor = function(element, unit, min, max, powerType, alt)
			local self = element._owner
			...
			if (UnitAffectingCombat("player") or UnitAffectingCombat("pet")) then
				local chargingAlpha = element.alpahEmptyRunes or element.alphaEmpty or .5
				local fullAlpha = 1

				for i = 1,#element.runeOrder do
					local runeID = element.runeOrder[i]
					local point = element[i]
					if (point) then
						if (not point.runeType) then
							point.runeType = GetRuneType(element.runeOrder[i])
						end
			...
			elseif (min < max) or (UnitExists("target") and not UnitIsFriend("player", "target")) then
				local chargingAlpha = (element.alpahEmptyRunes or element.alphaEmpty or .5)*(element.alphaNoCombatRunes or element.alphaNoCombat or .5)
...
local Enable = function(self)
	...
	if element then
		...
		if (IsRetail or IsWrath) then
			...
			if (IsWrath and PLAYERCLASS == "DEATHKNIGHT") then
				self:RegisterEvent("RUNE_TYPE_UPDATE", Proxy, true)
			end
		end
...
local Disable = function(self)
	...
	if element then
		...
		if (IsRetail or IsWrath) then
			...
			if (IsWrath and PLAYERCLASS == "DEATHKNIGHT") then
				self:UnregisterEvent("RUNE_TYPE_UPDATE", Proxy)
			end
		end
	end
end
